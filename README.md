...
| `lyra/` | The Lyra platform bundle: `manifest.json` plus the platform component mods under `platform/` (`mods-tab`, the mod-management UI; `wifi-power`, the Wi-Fi keep-alive). Versioned and updated as one entity. |
| `mods/` | Feature mods: `marketplace-redirect` (redirects the Marketplace endpoints to the [**ZuneNet community servers**](https://github.com/ZuneDev/ZuneNetApi)), `night-mode` (a quick toggle that washes the UI red), `screencast` (mirror the screen over Wi-Fi), `zune-cast` (stream audio to a Chromecast). `youtube` lives here too but is marked experimental: music works, video is unfinished. |
